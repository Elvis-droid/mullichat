...
/* =========================================================================
   SENT-MESSAGE BUTTERFLY
   A small flourish shown on every successful send: a butterfly rises
   and drifts off-screen, then fades. Each screen keeps a numeric
   "trigger" counter and increments it on send; bumping the number
   (even to the same value type) re-plays the animation via the
   effect's dependency below. Uses the native driver, so it's cheap
   even on lower-end Android 10 devices.
   ========================================================================= */

function SentButterfly({ triggerKey }: { triggerKey: number }) {
  const translateY = useRef(new Animated.Value(0)).current;
  const translateX = useRef(new Animated.Value(0)).current;
  const opacity = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    if (triggerKey === 0) return; // skip the initial mount
...
function MeshChatScreen() {
  ...
  return (
    ...
    >
      ...
      <SentButterfly triggerKey={sentTrigger} />
    </KeyboardAvoidingView>
  );
}
